...
def build_argparser():
  ...
  parser.add_argument("-i", "--input",
                      help="Address to the video live strem.", required=True,
                      type=str)
  parser.add_argument("-c", "--count",
                      help="Number of frames to process.", required=True,
                      type=int)
  ...
  parser.add_argument("-o", "--output_dir", help="Localtion for output video",
                      default="results/", type=str)
  return parser

def processBoxes(frame_count, res, labels_map, prob_threshold, frame, result_list, det_time):
  frame_res=[]
  for obj in res[0][0]:
    dims = ""
    ...
    if obj[2] > prob_threshold:
      ...
      class_id = int(obj[1])
      est = int(round(obj[2]*100, 1))
      time = int(round(det_time*1000))
      out_list = [frame_count, xmin, ymin, xmax, ymax, class_id, est, time]
      frame_res.append(out_list)
  result_list.append(tuple([frame, frame_res]))

def placeBoxes(res, labels_map, frame, is_async_mode):
  for obj in res:
    frame_count, xmin, ymin, xmax, ymax, class_id, est, det_time = tuple(obj)
    ...
    cv2.putText(frame, async_mode_message, (10, int(frame.shape[0] - 20)), cv2.FONT_HERSHEY_COMPLEX, 0.5,(10, 10, 200), 1)
  return frame
...
def postProcess(result_list, width, height, labels_map, out_path, is_async_mode, ren_progress_file_path=None):
  ...
  # 0x00000021,
  vw = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc(*"MPEG"), 30.0, (width, height), True)
